# raymond [![Build Status](https://secure.travis-ci.org/aymerick/raymond.svg?branch=master)](http://travis-ci.org/aymerick/raymond) [![GoDoc](https://godoc.org/github.com/davidmdm/handlebars?status.svg)](http://godoc.org/github.com/davidmdm/handlebars)

Handlebars for [golang](https://golang.org) with the same features as [handlebars.js](http://handlebarsjs.com) `3.0`.

The full API documentation is available here: <http://godoc.org/github.com/davidmdm/handlebars>.

![Raymond Logo](https://github.com/davidmdm/handlebars/blob/master/raymond.png?raw=true 'Raymond')
...
## Quick Start
    ...
    $ go get github.com/davidmdm/handlebars
...
```html
{{#equal foo "bar"}}foo is bar{{/equal}} {{#equal foo baz}}foo is the same as baz{{/equal}} {{#equal nb
0}}nothing{{/equal}} {{#equal nb 1}}there is one{{/equal}} {{#equal nb "1"}}everything is stringified before
comparison{{/equal}}
```
...
```html
foo is bar foo is the same as baz there is one everything is stringified before comparison
```
...
```html
{{#each users as |user userId|}} {{#each user.books as |book bookId|}} User: {{userId}} Book: {{bookId}} {{/each}}
{{/each}}
```
...
```html
User: marcel Book: book1 User: marcel Book: book2 User: didier Book: bookA User: didier Book: bookB
```
